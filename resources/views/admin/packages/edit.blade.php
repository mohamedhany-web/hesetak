...
@section('page_title', 'تعديل باقة برامج')
                ...
                <div>
                    ...
                    <select id="track" name="track" class="{{ $fieldClass }}">
                        ...
                        @foreach(\App\Models\Package::trackLabels() as $key => $label)
                            <option value="{{ $key }}" @selected(old('track', $package->track) === $key)>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                ...
                @if($package->features && count($package->features) > 0)
                    @foreach($package->features as $feature)
                        <div class="flex gap-2">
                            <input type="text" name="features[]" value="{{ $feature }}" class="h-11 flex-1 rounded-xl border border-line px-4 text-sm focus:border-accent focus:outline-none focus:ring-2 focus:ring-accent/20">
                            <button type="button" onclick="removeFeature(this)" class="inline-flex h-11 w-11 items-center justify-center rounded-xl border border-line text-rose-600 {{ count($package->features) == 1 ? 'hidden' : '' }}"><i class="fas fa-times"></i></button>
                        </div>
                    @endforeach
                @else
                    <div class="flex gap-2">
                        <input type="text" name="features[]" class="h-11 flex-1 rounded-xl border border-line px-4 text-sm focus:border-accent focus:outline-none focus:ring-2 focus:ring-accent/20" placeholder="ميزة">
