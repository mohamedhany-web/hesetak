...
@php
  ...
  $icons = ['database', 'file-lines', 'lock', 'share-nodes', 'user-check', 'cookie-bite', 'arrows-rotate'];
@endphp

<section class="mc-page-hero">
  <div class="mc-container">
    <p class="mc-eyebrow">{{ __('public.privacy_page_title') }}</p>
    <h1>
      {{ $isRtl ? 'سياسة الخصوصية على' : 'Privacy policy on' }}
      <span class="mc-contact-hero__accent">{{ $brand }}</span>
    </h1>
    <p class="mc-lead">{{ __('public.legal_privacy_hero_sub') }}</p>
    <div class="mc-hero__actions">
      ...
      <a href="{{ route('public.terms') }}" class="mc-btn mc-btn--lg mc-btn--outline">{{ __('public.terms_page_title') }}</a>
    </div>
  </div>
</section>
...
<section class="mc-section mc-section--tight">
  <div class="mc-container">
    <div class="mc-legal-intro">
      <span class="mc-legal-intro__icon" aria-hidden="true"><i class="fas fa-shield-halved"></i></span>
      <p>{!! nl2br(e(__('public.legal_privacy_intro', ['brand' => $brand]))) !!}</p>
    </div>
  </div>
</section>
  ...
  <div class="mc-container">
    <div class="mc-legal-grid">
      @foreach(range(1, 7) as $i)
        <article class="mc-legal-card {{ $i === 7 ? 'mc-legal-card--wide' : '' }}">
          ...
          <p>{!! nl2br(e(__('public.legal_privacy_s'.$i.'_body', ['brand' => $brand]))) !!}</p>
        </article>
      @endforeach
    </div>
  </div>
  ...
  <div class="mc-container">
    <div class="mc-cta">
      <div>
        ...
        <p>{{ __('public.legal_privacy_cta_desc') }}</p>
      </div>
